...
class Tcoconut extends TObjetStd
{
	/**
	 * Draft status
	 */
	...
	public function __construct()
	{
		...
		$this->set_table(MAIN_DB_PREFIX.'coconut');
...
//		$this->setChild('TcoconutChild','fk_coconut');
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$numero = get_next_value($db, $mask, 'coconut', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showcoconut") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/coconut/card.php', 1).'?id='.$this->getId(). $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='coconutStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='coconutStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='coconutStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='coconutStatusAccepted'; $shortkeytrans='Accepted'; }
 ...
 */
class Coconut extends Tcoconut
{
	private $PDOdb;
...
/*
class TcoconutChild extends TObjetStd
{
	public function __construct()
	{
		$this->set_table(MAIN_DB_PREFIX.'coconut_child');
		
		$this->add_champs('fk_coconut', array('type' => 'integer', 'index' => true));
